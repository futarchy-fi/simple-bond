...
contract SimpleBondV5 {
    ...
    // When a terminal outcome needs to refund the unresolved tail of the FIFO
    // queue, refunds are claimed in bounded batches rather than an unbounded
    // settlement loop that can be DoSed by a large challenger set.
    ...
    function withdrawBond(uint256 bondId) external {
        ...
        require(!b.conceded, "Claim conceded");
        require(msg.sender == b.poster, "Only poster");
        // The bond commitment must remain live until the public challenge
