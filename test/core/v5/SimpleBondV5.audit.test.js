...
describe("SimpleBondV5 audit coverage", function () {
  ...
  describe("ManualJudge fee recovery (M-01 fix)", function () {
    it("operator can withdraw accumulated fees via withdrawFees", async function () {
      await createDefaultBond();
      await bond.connect(challenger1).challenge(0, "C1");
      await bond.connect(challenger2).challenge(0, "C2");
      ...
      const judgeBalance = await token.balanceOf(judgeAddr);
      expect(judgeBalance).to.equal(JUDGE_FEE * 2n);

      const operatorBefore = await token.balanceOf(judgeOperator.address);
      await manualJudge.connect(judgeOperator).withdrawFees(
        fixture.addresses.token, judgeOperator.address, judgeBalance
      );
  ...
  describe("High challenge count refund loop (M-02)", function () {
    ...
    it("refunds 50 challengers via concede without reverting", async function () {
      ...
      // Concede — enables pull-based refunds for all challengers.
      await expect(
        manyFixture.actions.concede({ metadata: "Mass refund" })
      ).to.not.be.reverted;

      await manyFixture.actions.claimAllRefunds();
    ...
    it("ruleForChallenger refunds remaining challengers after partial poster wins", async function () {
      ...
      // Challenger wins #4 — enables refund claims for the remaining queue.
      await expect(
