...
async function createBondWithSeededChallenges(fixture, nextUint32, minimumChallenges = 1) {
  ...
  for (let index = 0; index < challengeCount; index += 1) {
    ...
    expectOperatorBalanceUnchanged(previous, current);
    previous = current;
  }
...
async function runSeededPosterFlow(seed) {
  const fixture = await deploySimpleBondV5FuzzFixture();
  ...
  if (posterWins === challengeCount) {
    ...
    expectOperatorBalanceUnchanged(previous, afterWithdraw);
    return;
  }
  ...
  if (terminalAction === 0) {
    ...
    expectOperatorBalanceUnchanged(previous, afterChallengerWin);
    return;
  }
  ...
  if (terminalAction === 1) {
    ...
    expectOperatorBalanceUnchanged(previous, afterReject);
    return;
  }
  ...
  expectOperatorBalanceUnchanged(previous, afterTimeout);
}
...
async function runSeededConcedeFlow(seed) {
  ...
  expectOperatorBalanceUnchanged(previous, afterConcede);
}

describe("SimpleBondV5 seeded fuzz flows", function () {
