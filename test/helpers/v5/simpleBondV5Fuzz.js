...
async function deploySimpleBondV5FuzzFixture(options = {}) {
  ...
  const actions = {
    ...
    async advancePastDeadline({ bondId = 0 } = {}) {
      ...
      const target = bondState.deadline > latest ? Number(bondState.deadline) : latest;
      await time.increaseTo(target + 1);

      return { bondId, timestamp: target + 1 };
    },
    ...
    async claimAllRefunds({ bondId = 0, maxCountPerTx = 50, caller = actors.outsider } = {}) {
      ...
      while (true) {
        ...
        const batchSize = remaining < BigInt(maxCountPerTx) ? remaining : BigInt(maxCountPerTx);
        const { receipt } = await actions.claimRefunds({ bondId, maxCount: batchSize, caller });
